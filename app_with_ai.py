...
# Set OpenAI API Key
OPENAI_API_KEY = "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"
...
class Interview:
    def __init__(self, id, user_id, job_role, resume_uploaded, questions_answered, total_questions, duration_minutes, communication_score, confidence_score, relevance_score, final_score, created_at):
        self.id = id
...
class Feedback:
    def __init__(self, id, interview_id, question_number, question, answer, communication, confidence, relevance, grammar, suggestion, created_at):
        self.id = id
...
def analyze_answer_with_ai(question, answer):
    """Analyze answer using OpenAI API"""
    try:
        ...
        prompt = f"""
        You are an expert interview coach. Analyze the following interview answer and provide detailed feedback.

        Question: {question}
        Answer: {answer}

        Please analyze the answer and provide feedback in the following format:
        - Communication Quality: (Excellent/Good/Fair/Poor)
        - Confidence Level: (High/Medium/Low)
        - Relevance Score: (1-10, where 10 is most relevant)
        - Grammar Quality: (Excellent/Good/Needs Improvement)
        ...
        feedback = {
            ...
            'relevance': 7,
            'grammar': 'Good',
            'suggestion': 'Good answer! Consider providing more specific examples.'
        }
        
        # Try to extract structured information
        for line in lines:
            if 'Communication Quality:' in line:
                feedback['communication'] = line.split(':')[-1].strip()
            elif 'Confidence Level:' in line:
                feedback['confidence'] = line.split(':')[-1].strip()
            elif 'Relevance Score:' in line:
                try:
                    score = int(line.split(':')[-1].strip())
                    feedback['relevance'] = min(10, max(1, score))
                except:
                    feedback['relevance'] = 7
            elif 'Grammar Quality:' in line:
                feedback['grammar'] = line.split(':')[-1].strip()
            elif 'Suggestion:' in line:
                feedback['suggestion'] = line.split(':')[-1].strip()
...
def submit_answer():
    ...
    feedback = analyze_answer_with_ai(question, answer)
...
def summary():
    ...
    # Calculate scores
    if feedback_list:
        avg_comm = sum(7 for f in feedback_list) / len(feedback_list)
        avg_conf = sum(7 for f in feedback_list) / len(feedback_list)
...
def test_openai():
    ...
    try:
        ...
        response = openai.ChatCompletion.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "user", "content": "Hello, can you provide a brief response?"}
            ],
            max_tokens=50
        )
        return jsonify({
...
if __name__ == '__main__':
    print("🚀 InterviewAI with OpenAI Integration Starting...")
